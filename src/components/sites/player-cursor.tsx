...
import type { ScrollEvent } from 'types/event';
...
export class PlayerCursor extends React.Component<Props> {
  ...
  private get cursorPosition() {
    return last(this.props.cursor) || [0, 0];
  }
  ...
  private get cursorScrollX() {
    return this.props.scroll[0];
  }

  private get cursorScrollY() {
  ...
  public render(): JSX.Element {
    return (
      <>
        ...
        <svg id='cursor-trail' style={{ width: this.cursorBoundX, height: this.cursorBoundY, transform: `translate(-${this.cursorScrollX}px, -${this.cursorScrollY}px)` }}>
          <path
            d={this.trail}
            fill='transparent'
